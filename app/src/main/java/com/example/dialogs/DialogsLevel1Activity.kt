...
class DialogsLevel1Activity : AppCompatActivity() {
    ...
    private fun showAlertDialog() {
        val listener = DialogInterface.OnClickListener { _, which -> // _ - dialog, which - returns a key of pressed button (positive, negative, neutral
            when (which) {
        ...
        val dialog = AlertDialog.Builder(this)
            .setCancelable(true) //default(if not called) true, if false then dialog can not be cancelled by clicking out of dialog window or back pressed
            .setIcon(R.mipmap.ic_launcher_round) // set an icon of dialog
            .setTitle(R.string.default_alert_title) // set a title of dialog
            .setMessage(R.string.default_alert_message) //set a message inside dialog
            .setPositiveButton(R.string.action_yes, listener) //displays and set positive button, necessary needs onClick listener
            .setNegativeButton(R.string.action_no, listener) //displays and set negative button, necessary needs onClick listener
            .setNeutralButton(R.string.action_ignore, listener) //displays and set neutral button, necessary needs onClick listener
            .setOnCancelListener { //method works when user cancel dialog
              showToast(R.string.dialog_cancelled)
            }
            .setOnDismissListener { //works each time dialog is closed
    ...
    private fun showSingleChoiceAlertDialog() {
        val volumeItems = AvailableVolumeValues.createVolumeValues(volume) // list of values
        val volumeTextItems = volumeItems.values
                .map { getString(R.string.volume_description, it) } //transform each int to string
                .toTypedArray() //transform list to Array<String>

        val dialog = AlertDialog.Builder(this)
            .setTitle(R.string.volume_setup)
            .setSingleChoiceItems(volumeTextItems // 1st param - Array, ArrayRes, adapter or cursor
                , volumeItems.currentIndex)       // 2nd param - current index to set chosen element
                { dialog, which ->                // 3rd element - listener
    ...
    private fun showSingleChoiceWithConfirmationAlertDialog() {
        ...
        val dialog = AlertDialog.Builder(this)
            ...
            .setPositiveButton(R.string.action_confirm) { d, _ ->
                val index = (d as AlertDialog).listView.checkedItemPosition //get index of selected position
    ...
    private fun showMultipleChoiceAlertDialog() {
        ...
        val dialog = AlertDialog.Builder(this)
                .setTitle(R.string.volume_setup)
                .setMultiChoiceItems(colorItems //array, ResArray or cursor of elements
                    , checkboxes)   //array that keep state of booleans (is element chosen or not)
                    { _, which, isChecked ->
